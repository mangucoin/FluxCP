...
<p><strong>Note:</strong> <?php echo htmlspecialchars(Flux::message('AccountConfirmEmailInfo')) ?></p>
<?php endif ?>
...
<form action="<?php echo $this->url ?>" method="post" class="generic-form">
	...
	<table class="generic-form-table">
		...
		<tr>
			...
			<td><input type="text" name="username" id="register_username" value="<?php echo htmlspecialchars($params->get('username') ?: '') ?>" autocomplete="username" /></td>
		</tr>

		<tr>
			<th><label for="register_password"><?php echo htmlspecialchars(Flux::message('AccountPasswordLabel')) ?></label></th>
			<td><input type="password" name="password" id="register_password" autocomplete="new-password" /></td>
		</tr>

		<tr>
			<th><label for="register_confirm_password"><?php echo htmlspecialchars(Flux::message('AccountPassConfirmLabel')) ?></label></th>
			<td><input type="password" name="confirm_password" id="register_confirm_password" autocomplete="new-password" /></td>
		</tr>
		...
		<tr>
			...
			<td><div class="red"><?php echo htmlspecialchars(Flux::message('AccountRecaptchaKey')) ?></div></td>
			<?php elseif (Flux::config('EnableReCaptcha')): ?>
			<th><label><?php echo htmlspecialchars(Flux::message('AccountSecurityLabel')) ?></label></th>
			<td><div class="g-recaptcha" data-theme="dark" data-sitekey="<?php echo $recaptcha ?>"></div></td>
			<?php else: ?>
			...
			<td>
				...
				<div class="action">
					<a href="javascript:refreshSecurityCode('.security-code img')"><?php echo htmlspecialchars(Flux::message('RefreshSecurityCode')) ?></a>
				</div>
			</td>
			<?php endif ?>
		</tr>
